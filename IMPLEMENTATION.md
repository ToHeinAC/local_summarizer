...
- **Summarization only**: the app has one job and one window — no sections, no
  sidebar mode switch. A stock-portfolio tracker briefly lived here as a second
  sidebar section (phase 4, commit `2f1fa12`) and was removed again: it shared no
  domain, no data and no dependency with summarization, and it forced a mode
  radio onto an otherwise single-purpose tool. Its home is the sibling
  **hundred-bagger** app, whose DuckDB schema was already designed around exactly
  that feature (its Phase 4). Dropping it also removed this app's only network
  dependency (`yfinance`), so "fully offline" is unqualified again — do not
  reintroduce a network call here without revisiting that claim.
...
- **Plain-text conversion (default) + precision selectbox**: the UI converts with
  ...
  on a small fixed model (`REWRITE_MODEL`, default `gemma3:4b`) — independent
  of the summarization model, so the model selector only affects
  summarization. It was `LiquidAI/lfm2.5-1.2b-instruct:latest` until that model
  started throwing a 500 ("peg-native format") from Ollama's structured-output
  grammar on some rewrite calls — see [docs/ingestion.md](docs/ingestion.md).
...
- **Cost**: because the UI converts with `fast=True`, a digital PDF adds **no**
  ...
  call per page) uses the small `REWRITE_MODEL`, so even the precise path stays
  cheap relative to a summarization-sized model, and it never rewords. See
