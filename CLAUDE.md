...
## Project at a glance
Local, offline document summarizer: Streamlit UI → LangGraph map-reduce agent → local Ollama LLM → Markdown summary downloadable as `.md` / `.pdf` / `.docx`. No cloud APIs, no embeddings, no vector DB.

```bash
uv sync
uv run streamlit run src/app.py --server.port 8506   # port per PRD.md
uv run pytest                                        # 46 tests, fully offline
```

**Hard boundary:** LangChain/LangGraph may be imported *only* by `src/agent.py` and `src/tools.py`. Every other module is plain Python. All prompt strings are named constants in `src/prompts.py`.
...
- [docs/ingestion.md](docs/ingestion.md) @docs/ingestion.md — PDF/DOCX/TXT/MD extraction, language detection
