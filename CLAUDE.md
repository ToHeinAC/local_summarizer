...
## Project at a glance
Local, offline document summarizer: Streamlit UI → Markdown conversion (vision OCR for scanned PDFs) → LangGraph map-reduce agent → local Ollama LLM → Markdown summary downloadable as `.md` / `.pdf` / `.docx`. No cloud APIs, no embeddings, no vector DB. Summarization is all the app does: one window, no sections, no mode switch.

```bash
uv sync
uv run streamlit run src/app.py --server.port 8530   # port per PRD.md
uv run pytest                                        # 116 tests, fully offline
```
