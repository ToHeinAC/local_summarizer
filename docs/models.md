...
|---|---|---|---|---|
| `fast` | `gemma4:e2b` | Fast | 3 | 1 |
| `standard` *(default)* | `gemma4:e4b` | Standard | 2 | 2 |
| `smarter` | `qwen3:14b` | Smarter | 1 | 3 |
| `accurate` | `gpt-oss:20b` | Accurate | 1 | 3 |

`speed`/`quality` are 1-3 metrics rendered as stars in the UI. Tags match the
PRD and the user's `ollama list`.
...
- `annotate_availability(host) -> list[dict]` — each model plus an `installed`
  bool. The UI warns and disables **Summarize** for uninstalled models,
